...
status([]) ->
    try
        case pylon_status:statistics() of
            [] ->
                io:format("pylon_stat is not enabled.\n", []);
            Stats ->
...
vnode_status([]) ->
    try
        case pylon_status:vnode_status() of
            [] ->
...
reload_code([]) ->
    case app_helper:get_env(pylon, add_paths) of
